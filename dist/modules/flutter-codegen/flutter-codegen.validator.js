...
exports.DEFAULT_FLUTTER_API_BASE_URL = exports.DEFAULT_FLUTTER_APP_NAME = void 0;
exports.validateFlutterCodegenRequest = validateFlutterCodegenRequest;
const http_error_1 = require("../../utils/http-error");
exports.DEFAULT_FLUTTER_APP_NAME = 'mi_flutter_app';
exports.DEFAULT_FLUTTER_API_BASE_URL = 'http://192.168.31.77:8080';
/**
 ...
 */
function validateFlutterCodegenRequest(body) {
    if (!isRecord(body)) {
        throw new http_error_1.HttpError('El cuerpo de la petición debe ser un objeto JSON', 400);
    }
    const appName = optionalString(body.app_name) || exports.DEFAULT_FLUTTER_APP_NAME;
    const apiBaseUrl = optionalString(body.api_base_url) || exports.DEFAULT_FLUTTER_API_BASE_URL;
...
function validateDiagram(value) {
    if (!isRecord(value)) {
        throw new http_error_1.HttpError('El campo "diagram" es obligatorio y debe ser un objeto', 400);
    }
    if (!isRecord(value.classes) || Array.isArray(value.classes)) {
        throw new http_error_1.HttpError('El campo "diagram.classes" es obligatorio y debe ser un objeto', 400);
    }
    const classes = {};
    for (const [key, candidate] of Object.entries(value.classes)) {
        if (!isRecord(candidate) || typeof candidate.id !== 'string' || !candidate.id.trim()) {
            throw new http_error_1.HttpError(`La clase "${key}" debe tener un id válido`, 400);
        }
        if (typeof candidate.name !== 'string' || !candidate.name.trim()) {
            throw new http_error_1.HttpError(`La clase "${key}" debe tener un nombre válido`, 400);
        }
...
function normalizeAttributes(value, className) {
    if (!Array.isArray(value)) {
        throw new http_error_1.HttpError(`Los atributos de "${className}" deben ser iterables`, 400);
    }
    return value.map((candidate, index) => {
        if (!isRecord(candidate) || typeof candidate.name !== 'string' || !candidate.name.trim()) {
            throw new http_error_1.HttpError(`El atributo ${index + 1} de "${className}" debe tener un nombre válido`, 400);
        }
...
function validateRelations(value, classes) {
    if (value === undefined)
        return {};
    if (!isRecord(value) || Array.isArray(value)) {
        throw new http_error_1.HttpError('El campo "diagram.links" debe ser un objeto', 400);
    }
    ...
    for (const [key, candidate] of Object.entries(value)) {
        if (!isRecord(candidate) ||
            ...
            throw new http_error_1.HttpError(`La relación "${key}" tiene una estructura inválida`, 400);
        }
        if (!classIds.has(candidate.sourceId)) {
            throw new http_error_1.HttpError(`La relación "${key}" referencia un sourceId inexistente`, 400);
        }
        if (!classIds.has(candidate.targetId)) {
            throw new http_error_1.HttpError(`La relación "${key}" referencia un targetId inexistente`, 400);
        }
        links[key] = {
            id: candidate.id,
            kind: candidate.kind,
