...
import { listZipEntries } from '../../codegen/tests/zip.utils'
...
async function main(): Promise<void> {
  const zip = await generateFlutterZip(request)
  assert.strictEqual(zip.readUInt32LE(0), 0x04034b50, 'el resultado debe ser un ZIP válido')

  const paths = listZipEntries(zip).map((entry) => entry.path)
  for (const path of [
    'lib/main.dart',
    ...
    assert(paths.includes(path), `falta ${path} en el ZIP`)
  }

  assert(!paths.some((path) => path === 'pubspec.yaml' || path.startsWith('android/')))
