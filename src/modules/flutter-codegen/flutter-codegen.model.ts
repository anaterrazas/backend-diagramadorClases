...
const TYPE_MAP: Record<string, { dartType: string; jsonType: string }> = {
...
const CRUD_OPERATIONS: FlutterModule['operations'] = [
  'list',
  'getById',
  'create',
  'update',
...
export function buildFlutterProject(request: FlutterCodegenRequest): FlutterProject {
...
function buildFlutterClass(
  ...
  const fields = (umlClass.attributes ?? []).map((attribute) =>
    buildFlutterField(attribute, classByName),
  )
  ...
  for (const relation of linkRelations) {
    if (!relation.isGeneratedField || fields.some((field) => field.dartName === relation.fieldName)) continue
    fields.push({
...
function buildFlutterField(
  attribute: UMLAttribute,
  classByName: Map<string, UMLClass>,
): FlutterField {
...
function buildFlutterModule(item: FlutterClass): FlutterModule {
...
function relationClassNameFromField(
  fieldName: string,
  classByName: Map<string, UMLClass>,
): string | undefined {
  if (!fieldName.toLowerCase().endsWith('id')) return undefined
  const candidate = fieldName.slice(0, -2).toLowerCase()
  return classByName.get(candidate)?.name
}

function uniqueRelations(relations: FlutterRelation[]): FlutterRelation[] {
